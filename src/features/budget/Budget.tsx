  ...
  return <div style={{ animation: "fadeUp .4s ease both" }}>
    <div style={{ position: "relative", overflow: "hidden", borderRadius: "var(--r-5)", padding: 24, background: "linear-gradient(125deg,var(--ac,#2a7089),#195866)", color: "#fff", marginBottom: 18 }}>
      ...
      <div style={{ position: "relative", display: "flex", justifyContent: "space-between", gap: 18, alignItems: "flex-end", flexWrap: "wrap" }}>
        ...
        <div style={{ textAlign: "right" }}><div style={{ display: "flex", justifyContent: "flex-end", gap: 4, marginBottom: 10 }}>{currencies.map((value) => <button key={value} onClick={() => selectCurrency(value)} disabled={value !== "EUR" && rates[value] <= 0} style={{ border: "none", borderRadius: "var(--r-pill)", padding: "5px 10px", cursor: value !== "EUR" && rates[value] <= 0 ? "wait" : "pointer", fontSize: 11, fontWeight: 700, background: displayCurrency === value ? "#fff" : "rgba(255,255,255,.16)", color: displayCurrency === value ? "var(--ac,#2a7089)" : "#fff", opacity: value !== "EUR" && rates[value] <= 0 ? .55 : 1 }}>{value}</button>)}</div><div style={{ fontSize: 12, letterSpacing: ".12em", textTransform: "uppercase", opacity: .76 }}>Общая сумма</div><div style={{ fontFamily: "'Mulish',system-ui,sans-serif", fontVariantNumeric: "tabular-nums", fontSize: 42, fontWeight: 700, lineHeight: 1, marginTop: 4 }}>{formatAmount(total)}</div></div>
      </div>
    </div>

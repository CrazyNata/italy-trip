  ...
  return <div style={{ animation: "fadeUp .4s ease both" }}>
    <div style={{ position: "relative", overflow: "hidden", borderRadius: "var(--r-5)", padding: 24, background: "linear-gradient(125deg,var(--ac,#b5623c),#8f4227)", color: "#fff", marginBottom: 18 }}>
      ...
      <div style={{ position: "relative", display: "flex", justifyContent: "space-between", gap: 18, alignItems: "flex-end", flexWrap: "wrap" }}>
        ...
        <div style={{ textAlign: "right" }}><div style={{ display: "flex", justifyContent: "flex-end", gap: 4, marginBottom: 10 }}>{currencies.map((value) => <button key={value} onClick={() => selectCurrency(value)} disabled={value !== "EUR" && rates[value] <= 0} style={{ border: "none", borderRadius: "var(--r-2)", padding: "5px 10px", cursor: value !== "EUR" && rates[value] <= 0 ? "wait" : "pointer", fontSize: 11, fontWeight: 700, background: displayCurrency === value ? "#fff" : "rgba(255,255,255,.16)", color: displayCurrency === value ? "var(--ac,#2a7089)" : "#fff", opacity: value !== "EUR" && rates[value] <= 0 ? .55 : 1 }}>{value}</button>)}</div><div style={{ fontSize: 12, letterSpacing: ".12em", textTransform: "uppercase", opacity: .76 }}>Общая сумма</div><div style={{ fontFamily: "'Mulish',system-ui,sans-serif", fontVariantNumeric: "tabular-nums", fontSize: 42, fontWeight: 700, lineHeight: 1, marginTop: 4 }}>{formatAmount(total)}</div></div>
      </div>
    </div>
    <div className="budget-summary-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 12, marginBottom: 18 }}>
      <button className="budget-summary-card" type="button" onClick={() => setExpandedBreakdowns((current) => ({ ...current, family: !current.family }))} aria-expanded={expandedBreakdowns.family} style={{ border: "1px solid var(--line,#e7dcc7)", borderRadius: "var(--r-3)", padding: "16px 18px", background: "var(--card,#fff)", textAlign: "left", color: "inherit" }}><div style={{ display: "flex", alignItems: "center", gap: 12 }}><span style={{ width: 34, height: 34, borderRadius: "var(--r-2)", background: "var(--soft,#f1f7f6)", color: "var(--ac,#2a7089)", display: "grid", placeItems: "center" }}><i className="fa-solid fa-people-group" /></span><div style={{ flex: 1 }}><div style={{ fontSize: 11, letterSpacing: ".12em", textTransform: "uppercase", color: "var(--muted,#8a7d6b)", fontWeight: 700 }}>На семью</div><div style={{ fontFamily: "'Mulish',system-ui,sans-serif", fontVariantNumeric: "tabular-nums", fontSize: 27, fontWeight: 700, lineHeight: 1.15, marginTop: 3 }}>{formatAmount(total / familySize)}</div></div><i className="fa-solid fa-chevron-down" style={{ color: "var(--muted,#8a7d6b)", fontSize: 12, transform: expandedBreakdowns.family ? "rotate(180deg)" : undefined, transition: "transform .2s" }} /></div>{expandedBreakdowns.family && <div className="budget-summary-card-detail">{breakdown(familySize, "var(--ac,#2a7089)")}</div>}</button>
      {expandedBreakdowns.family && <div className="budget-summary-mobile-detail">{breakdown(familySize, "var(--ac,#2a7089)")}</div>}
      <button className="budget-summary-card" type="button" onClick={() => setExpandedBreakdowns((current) => ({ ...current, day: !current.day }))} aria-expanded={expandedBreakdowns.day} style={{ border: "1px solid var(--line,#e7dcc7)", borderRadius: "var(--r-3)", padding: "16px 18px", background: "var(--card,#fff)", textAlign: "left", color: "inherit" }}><div style={{ display: "flex", alignItems: "center", gap: 12 }}><span style={{ width: 34, height: 34, borderRadius: "var(--r-2)", background: "#f7eee1", color: "var(--ac2,#d99a4e)", display: "grid", placeItems: "center" }}><i className="fa-regular fa-calendar" /></span><div style={{ flex: 1 }}><div style={{ fontSize: 11, letterSpacing: ".12em", textTransform: "uppercase", color: "var(--muted,#8a7d6b)", fontWeight: 700 }}>В день</div><div style={{ fontFamily: "'Mulish',system-ui,sans-serif", fontVariantNumeric: "tabular-nums", fontSize: 27, fontWeight: 700, lineHeight: 1.15, marginTop: 3 }}>{formatAmount(total / days)}</div></div><i className="fa-solid fa-chevron-down" style={{ color: "var(--muted,#8a7d6b)", fontSize: 12, transform: expandedBreakdowns.day ? "rotate(180deg)" : undefined, transition: "transform .2s" }} /></div>{expandedBreakdowns.day && <div className="budget-summary-card-detail">{breakdown(days, "var(--ac2,#d99a4e)")}</div>}</button>
    ...
    {categories.length > 0 && <div style={{ background: "var(--card,#fff)", border: "1px solid var(--line,#e7dcc7)", borderRadius: "var(--r-5)", overflow: "hidden", marginBottom: 18 }}>
      ...
      <div style={{ padding: "18px 20px" }}>
        <div style={{ display: "flex", gap: 3, height: 14, borderRadius: "var(--r-1)", overflow: "hidden", marginBottom: 16 }}>{categories.map(([category, value]) => <div key={category} title={`${category} · ${formatAmount(value)}`} style={{ width: `${(value / total) * 100}%`, minWidth: 3, background: catColor(category) }} />)}</div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(168px,1fr))", gap: 10 }}>{categories.map(([category, value]) => {
          const percent = Math.round((value / total) * 100);
          return <div key={category} style={{ position: "relative", overflow: "hidden", border: "1px solid var(--line,#e7dcc7)", borderRadius: "var(--r-3)", padding: "11px 13px 14px", background: "var(--soft,#f1f7f6)" }}>
    ...
    <div style={{ background: "var(--card,#fff)", border: "1px solid var(--line,#e7dcc7)", borderRadius: "var(--r-5)", overflow: "hidden" }}>

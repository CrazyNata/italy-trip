...
const cityDayLabels: Record<string, Record<number, string>> = {
  ...
  "Милан, Италия": {
    1: "День 1: соборный Милан",
    2: "День 2: «Тайная вечеря» и Навильи",
  },
  "Озеро Комо, Италия": {
    1: "Круг вокруг озера на машине",
  },
...
function WalkingMap({
  ...
  useEffect(() => {
    ...
    void import("mapbox-gl").then(({ default: mapbox }) => {
      ...
      const routePoints = userLocation
        ? [userLocation, ...sights.map((sight) => sight.lnglat!)]
        : sights.map((sight) => sight.lnglat!);
      if (userLocation) {
        markers.push(
      ...
      map.on("load", async () => {
        ...
        if (sights.length + Number(Boolean(userLocation)) < 2) {
          onRoute(null, "");
          return;
        }
        const signature = [
          ...sights.map((sight) => `${sight.id}:${sight.lnglat!.join(",")}`),
          ...(userLocation ? [`you:${userLocation.join(",")}`] : []),
        ].join("|");
        try {
  ...
  return (
    <div ref={element} className="walk-route-map" style={{ height: 380, minHeight: 280, borderRadius: "var(--r-3)", overflow: "hidden", background: "var(--track,#efe4cf)", position: "relative" }}>
...
export function Sights() {
  ...
  const located = walkAll
    ...
    .slice(0, 25);
  const routeCoordinates = walkLocation
    ? [walkLocation, ...located.map((sight) => sight.lnglat!)]
    : located.map((sight) => sight.lnglat!);
  const mapUrl =
    routeCoordinates.length < 2
      ? ""
      : `https://www.google.com/maps/dir/?api=1&origin=${encodeURIComponent(`${routeCoordinates[0][1]},${routeCoordinates[0][0]}`)}&destination=${encodeURIComponent(`${routeCoordinates.at(-1)![1]},${routeCoordinates.at(-1)![0]}`)}${
          routeCoordinates.length > 2
  ...
  function findWalkLocation() {
    if (!navigator.geolocation)
      return toast("Геолокация недоступна в этом браузере");
    ...
    <>
      ...
      <section style={{ background: "radial-gradient(120% 90% at 0% 0%, rgba(42,112,137,.16), transparent 55%), radial-gradient(120% 90% at 100% 100%, rgba(217,154,78,.16), transparent 55%), var(--track,#efe4cf)", border: "1px solid var(--line,#e7dcc7)", borderRadius: "var(--r-4)", padding: 16, margin: "0 0 24px" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 13 }}>
          <div onClick={() => setCollapsed((prev) => ({ ...prev, route: !prev.route }))} title={collapsed.route ? "Развернуть маршрут" : "Свернуть маршрут"} style={{ cursor: "pointer", userSelect: "none" }}><div style={{ display: "flex", alignItems: "center", gap: 8 }}><div style={{ fontFamily: "'Playfair Display',serif", fontSize: 22, fontWeight: 600 }}><i className="fa-solid fa-person-walking" style={{ color: "var(--ac,#b95c3f)", fontSize: 18, marginRight: 7 }} />Пеший маршрут</div><span style={{ flex: 1 }} /><i className="fa-solid fa-chevron-down" style={{ fontSize: 14, color: "var(--muted,#8a7d6b)", transform: collapsed.route ? "rotate(-90deg)" : "none", transition: "transform .2s" }} /></div>{!collapsed.route && <div style={{ fontSize: 12.5, color: "var(--muted,#8a7d6b)", marginTop: 3 }}>Нажмите на пункт списка, чтобы открыть его на Google Картах. Порядок меняйте стрелками или перетаскиванием маркеров.</div>}</div>
          {!collapsed.route && <div className="walk-controls" style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
        ...
        {!collapsed.route && (activeCity ? <div className="walk-route-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(260px,1fr))", gap: 14 }}>
          <div className="walk-route-list" style={{ display: "flex", flexDirection: "column", gap: 7, maxHeight: 380, overflow: "auto" }}>
            {walkAll.map((sight, index) => { const active = walkFocus?.id === sight.id; return <div key={sight.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", border: `1px solid ${active ? "var(--ac,#b95c3f)" : "var(--line,#e7dcc7)"}`, borderRadius: "var(--r-2)", background: active ? "var(--card,#fff)" : "var(--soft,#fdfaf3)", transition: "border-color .2s, background .2s" }}><a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${sight.name}, ${sight.city}`)}`} target="_blank" rel="noreferrer" onClick={(event) => { event.preventDefault(); const href = event.currentTarget.href; if (sight.lnglat) { setWalkFocus({ id: sight.id, nonce: Date.now() }); window.setTimeout(() => window.open(href, "_blank", "noopener,noreferrer"), 1000); } else { window.open(href, "_blank", "noopener,noreferrer"); } }} title="Открыть на Google Картах" style={{ display: "flex", alignItems: "center", gap: 8, flex: 1, minWidth: 0, cursor: "pointer", textDecoration: "none", color: "inherit" }}><span style={{ width: 21, height: 21, flex: "none", borderRadius: "50%", display: "grid", placeItems: "center", background: "var(--ac,#b95c3f)", color: "#fff", fontSize: 11, fontWeight: 700 }}>{index + 1}</span><span style={{ flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: 13, fontWeight: 600, color: sight.lnglat ? undefined : "var(--muted,#8a7d6b)" }}>{sight.name}</span><i className="fa-solid fa-arrow-up-right-from-square" style={{ fontSize: 11, color: "var(--muted,#8a7d6b)", flex: "none" }} /></a><button onClick={() => move(sight.id, -1)} title="Раньше" style={{ border: "none", background: "none", color: "var(--muted,#8a7d6b)", cursor: "pointer", padding: 4 }}><i className="fa-solid fa-chevron-up" /></button><button onClick={() => move(sight.id, 1)} title="Позже" style={{ border: "none", background: "none", color: "var(--muted,#8a7d6b)", cursor: "pointer", padding: 4 }}><i className="fa-solid fa-chevron-down" /></button></div>; })}
            {!located.length && <div style={{ fontSize: 13, color: "var(--muted,#8a7d6b)", padding: "8px 2px" }}>У мест нет координат.</div>}
          ...
          <WalkingMap sights={located} readOnly={isReadOnly} focus={walkFocus} userLocation={walkLocation} onMove={(id, lnglat) => mutate(id, { lnglat })} onRoute={(next, error) => { setRoute(next); setRouteError(error); }} />
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", marginBottom: showAdd ? 12 : 20 }}>
        ...
        <select value={filter.city} onChange={(e) => setFilter({ ...filter, city: e.target.value })} style={{ border: "1px solid var(--line,#e7dcc7)", borderRadius: "var(--r-2)", padding: "8px 11px", fontSize: 13, background: "var(--card,#fff)", color: "var(--ink,#3b3228)", cursor: "pointer" }}><option value="" style={{ color: "var(--ink,#3b3228)" }}>все города</option>{cities.map((city) => <option key={city} value={city} style={{ color: "var(--ink,#3b3228)" }}>{city}</option>)}</select>
        ...
        <span style={{ flex: 1 }} />
        {!isReadOnly && <button onClick={() => setShowAdd((v) => !v)} style={{ display: "inline-flex", alignItems: "center", gap: 7, border: `1px solid ${showAdd ? "var(--ac,#b95c3f)" : "var(--line,#e7dcc7)"}`, background: showAdd ? "var(--ac,#b95c3f)" : "var(--card,#fff)", color: showAdd ? "#fff" : "var(--ink,#3b3228)", borderRadius: "var(--r-2)", padding: "8px 14px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}><i className={showAdd ? "fa-solid fa-xmark" : "fa-solid fa-plus"} style={{ fontSize: 12 }} />{showAdd ? "Свернуть" : "Добавить место"}</button>}
      </div>

      {showAdd && !isReadOnly && <div className="sights-controls" style={{ background: "radial-gradient(120% 90% at 0% 0%, rgba(42,112,137,.16), transparent 55%), radial-gradient(120% 90% at 100% 100%, rgba(217,154,78,.16), transparent 55%), var(--track,#efe4cf)", border: "1px solid var(--line,#e7dcc7)", borderRadius: "var(--r-4)", padding: "14px 16px", display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 20 }}>
      ...
      {(["обязательные", "необязательные"] as const).map((group) => {
        ...
        return (
          ...
          >
            <div onClick={() => setCollapsed((prev) => ({ ...prev, [group]: !prev[group] }))} title={collapsed[group] ? "Развернуть раздел" : "Свернуть раздел"} style={{ display: "flex", alignItems: "center", gap: 12, margin: "0 0 18px", paddingBottom: 11, borderBottom: "1.5px solid var(--line,#e7dcc7)", cursor: "pointer", userSelect: "none" }}><i className={group === "обязательные" ? "fa-solid fa-star" : "fa-regular fa-star"} style={{ color: group === "обязательные" ? "var(--ac,#b95c3f)" : "var(--muted,#8a7d6b)", fontSize: 16 }} /><h2 style={{ fontFamily: "'Playfair Display',serif", fontWeight: 600, fontSize: 25, margin: 0, letterSpacing: ".005em" }}>{group === "обязательные" ? "Обязательные" : "Необязательные"}</h2>{items.length > 0 && <span style={{ fontSize: 12, fontWeight: 700, color: "var(--muted,#8a7d6b)", background: "var(--track,#efe4cf)", borderRadius: "var(--r-2)", padding: "3px 11px" }}>{items.length} мест</span>}<span style={{ flex: 1 }} /><i className="fa-solid fa-chevron-down" style={{ fontSize: 13, color: "var(--muted,#8a7d6b)", transform: collapsed[group] ? "rotate(-90deg)" : "none", transition: "transform .2s" }} /></div>
            {!collapsed[group] && subs.map((sub) => {

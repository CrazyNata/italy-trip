...
export function Restaurants() {
  ...
  const cityMappedRestaurants = list.filter((item) => item.city === activeMapCity && item.lnglat);
   ...
   const mapUrl = mapRoutePoints.length > 1
     ? `https://www.google.com/maps/dir/?api=1&origin=${encodeURIComponent(mapRoutePoints[0].slice().reverse().join(","))}&destination=${encodeURIComponent(mapRoutePoints.at(-1)!.slice().reverse().join(","))}${mapRoutePoints.length > 2 ? `&waypoints=${encodeURIComponent(mapRoutePoints.slice(1, -1).map((point) => point.slice().reverse().join(",")).join("|"))}` : ""}`
  ...
  return (
    <>
      {cityMappedRestaurants.length > 0 && <section style={{ position: "relative", zIndex: mapFiltersOpen ? 3 : undefined, animation: "fadeUp .4s ease both", background: "radial-gradient(120% 90% at 0% 0%, rgba(42,112,137,.16), transparent 55%), radial-gradient(120% 90% at 100% 100%, rgba(217,154,78,.16), transparent 55%), var(--track,#efe4cf)", border: "1px solid var(--line,#e7dcc7)", borderRadius: "var(--r-4)", padding: 16, margin: "0 0 24px" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: mapCollapsed ? 0 : 13 }}>
          ...
          {!mapCollapsed && <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            ...
            <button type="button" onClick={findLocation} disabled={locating} title={userLoc ? "Местоположение найдено" : "Определить моё местоположение"} style={{ flex: "1 1 auto", justifyContent: "center", display: "inline-flex", alignItems: "center", border: "1px solid var(--line,#e7dcc7)", background: "var(--soft,#fdfaf3)", color: "var(--ink,#3b3228)", borderRadius: "var(--r-2)", padding: "8px 14px", fontSize: 13, fontWeight: 600, cursor: locating ? "wait" : "pointer" }}><i className={locating ? "fa-solid fa-spinner fa-spin" : userLoc ? "fa-solid fa-location-dot" : "fa-solid fa-location-crosshairs"} style={{ marginRight: 5 }} />{userLoc ? "Вы здесь" : "Моё местоположение"}</button>
            <div style={{ position: "relative", flex: "1 1 auto" }}><button type="button" onClick={() => setMapFiltersOpen((open) => !open)} style={{ width: "100%", justifyContent: "center", display: "inline-flex", alignItems: "center", gap: 6, border: "1px solid var(--line,#e7dcc7)", background: "var(--soft,#fdfaf3)", color: "var(--ink,#3b3228)", borderRadius: "var(--r-2)", padding: "8px 14px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}><i className="fa-solid fa-sliders" style={{ fontSize: 12 }} />Фильтры{mapCategories.length ? ` · ${mapCategories.length}` : ""}</button>{mapFiltersOpen && <div style={{ position: "absolute", zIndex: 10, top: "calc(100% + 7px)", left: 0, minWidth: "100%", width: 280, padding: 11, border: "1px solid var(--line,#e7dcc7)", borderRadius: "var(--r-2)", background: "var(--card,#fff)", boxShadow: "0 8px 24px rgba(59,50,40,.16)" }}><div style={{ fontSize: 11, fontWeight: 800, letterSpacing: ".1em", textTransform: "uppercase", color: "var(--muted,#8a7d6b)", marginBottom: 8 }}>Категории</div><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 5 }}>{restaurantCategories.map((category) => { const selected = mapCategories.includes(category); return <button key={category} type="button" onClick={() => toggleMapCategory(category)} style={{ border: `1px solid ${selected ? "var(--ac,#b95c3f)" : "var(--line,#e7dcc7)"}`, background: selected ? "var(--ac,#b95c3f)" : "var(--soft,#fdfaf3)", color: selected ? "#fff" : "var(--ink,#3b3228)", borderRadius: "var(--r-1)", padding: "6px 7px", fontSize: 11.5, fontWeight: 600, cursor: "pointer", textAlign: "left" }}>{category}</button>; })}</div></div>}</div>
        ...
        {!mapCollapsed && <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(260px,1fr))", gap: 14 }}><div style={{ display: "flex", flexDirection: "column", gap: 7, maxHeight: 380, overflow: "auto" }}>{mappedRestaurants.length > 0 ? <>{mappedRestaurants.map((item, index) => { const active = mapFocus === item.id; return <a key={item.id} href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${item.name}, ${item.city}`)}`} target="_blank" rel="noreferrer" onClick={(event) => { event.preventDefault(); const href = event.currentTarget.href; setMapFocus(item.id); window.setTimeout(() => window.open(href, "_blank", "noopener,noreferrer"), 1000); }} title="Открыть на Google Картах" style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", border: `1px solid ${active ? "var(--ac,#b95c3f)" : "var(--line,#e7dcc7)"}`, borderRadius: "var(--r-2)", background: active ? "var(--card,#fff)" : "var(--soft,#fdfaf3)", transition: "border-color .2s, background .2s", cursor: "pointer", textDecoration: "none", color: "inherit" }}><span style={{ width: 21, height: 21, flex: "none", borderRadius: "50%", display: "grid", placeItems: "center", background: "var(--ac,#b95c3f)", color: "#fff", fontSize: 11, fontWeight: 700 }}>{index + 1}</span><span style={{ flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: 13, fontWeight: 600 }}>{item.name}</span><i className="fa-solid fa-arrow-up-right-from-square" style={{ fontSize: 11, color: "var(--muted,#8a7d6b)", flex: "none" }} /></a>; })}<div style={{ fontSize: 12, color: "var(--muted,#8a7d6b)", padding: "3px 2px" }}>{mappedRestaurants.length} мест на карте</div></> : <div style={{ fontSize: 13, color: "var(--muted,#8a7d6b)", padding: "6px 0" }}>В этой категории нет ресторанов с координатами.</div>}</div><RestaurantCityMap restaurants={mappedRestaurants.length ? mappedRestaurants : cityMappedRestaurants} focus={mapFocus} googleMapsUrl={mapUrl} userLocation={userLoc} /></div>}
      ...
      {list.length > 0 && (
      <div className="filter-bar" style={{ display: "flex", flexWrap: "wrap", alignItems: "center", columnGap: 6, rowGap: 10, marginBottom: 16, padding: "12px 14px", borderRadius: "var(--r-3)", background: "radial-gradient(120% 90% at 0% 0%, rgba(42,112,137,.16), transparent 55%), radial-gradient(120% 90% at 100% 100%, rgba(217,154,78,.16), transparent 55%), var(--track,#efe4cf)", border: "1px solid var(--line,#e7dcc7)" }}>
        {cities.length > 1 && (
          <>

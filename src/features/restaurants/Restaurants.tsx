...
export function Restaurants() {
  ...
  return (
    <>
      ...
      {cityMappedRestaurants.length > 0 && <section style={{ animation: "fadeUp .4s ease both", background: "radial-gradient(120% 90% at 0% 0%, rgba(42,112,137,.16), transparent 55%), radial-gradient(120% 90% at 100% 100%, rgba(217,154,78,.16), transparent 55%), var(--track,#efe4cf)", border: "1px solid var(--line,#e7dcc7)", borderRadius: "var(--r-4)", padding: 16, margin: "0 0 24px" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: mapCollapsed ? 0 : 13 }}>
          ...
          {!mapCollapsed && <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            <select value={activeMapCity} onChange={(event) => { setMapCity(event.target.value); setMapFocus(null); }} style={{ minWidth: 150, maxWidth: 175, border: "1px solid var(--line,#e7dcc7)", borderRadius: "var(--r-2)", padding: "8px 10px", fontSize: 13, background: "var(--soft,#fdfaf3)", color: "var(--ink,#3b3228)" }}><option value="">город...</option>{cities.map((city) => <option key={city}>{city}</option>)}</select>
            <div style={{ position: "relative" }}><button type="button" onClick={() => setMapFiltersOpen((open) => !open)} style={{ display: "inline-flex", alignItems: "center", gap: 6, border: "1px solid var(--line,#e7dcc7)", background: mapFiltersOpen || mapCategories.length ? "var(--soft,#fdfaf3)" : "transparent", color: "var(--ink,#3b3228)", borderRadius: "var(--r-1)", padding: "5px 9px", fontSize: 11.5, fontWeight: 600, cursor: "pointer" }}><i className="fa-solid fa-sliders" style={{ fontSize: 11 }} />Фильтры{mapCategories.length ? ` · ${mapCategories.length}` : ""}</button>{mapFiltersOpen && <div style={{ position: "absolute", zIndex: 5, top: "calc(100% + 6px)", left: 0, width: 250, padding: 11, border: "1px solid var(--line,#e7dcc7)", borderRadius: "var(--r-2)", background: "var(--card,#fff)", boxShadow: "0 8px 24px rgba(59,50,40,.16)" }}><div style={{ fontSize: 11, fontWeight: 800, letterSpacing: ".1em", textTransform: "uppercase", color: "var(--muted,#8a7d6b)", marginBottom: 8 }}>Категории</div><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 5 }}>{restaurantCategories.map((category) => { const selected = mapCategories.includes(category); return <button key={category} type="button" onClick={() => toggleMapCategory(category)} style={{ border: `1px solid ${selected ? "var(--ac,#b95c3f)" : "var(--line,#e7dcc7)"}`, background: selected ? "var(--ac,#b95c3f)" : "var(--soft,#fdfaf3)", color: selected ? "#fff" : "var(--ink,#3b3228)", borderRadius: "var(--r-1)", padding: "6px 7px", fontSize: 11.5, fontWeight: 600, cursor: "pointer", textAlign: "left" }}>{category}</button>; })}</div></div>}</div>
            <button type="button" onClick={async () => { if (!mapUrl) return toast("Добавьте минимум два ресторана с координатами"); try { await navigator.clipboard.writeText(mapUrl); } catch {} setMapCopied(true); window.setTimeout(() => setMapCopied(false), 1800); }} style={{ display: "inline-flex", alignItems: "center", gap: 6, border: "1px solid var(--line,#e7dcc7)", background: "transparent", color: "var(--muted,#8a7d6b)", borderRadius: "var(--r-1)", padding: "5px 9px", fontSize: 11.5, fontWeight: 600, cursor: "pointer" }}><i className={mapCopied ? "fa-solid fa-check" : "fa-regular fa-copy"} style={{ fontSize: 11 }} />{mapCopied ? "Скопировано" : "Копировать маршрут"}</button>
          </div>}
        </div>
        {!mapCollapsed && (mappedRestaurants.length > 0 ? <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(260px,1fr))", gap: 14 }}><div style={{ display: "flex", flexDirection: "column", gap: 7, maxHeight: 380, overflow: "auto" }}>{mappedRestaurants.map((item, index) => { const active = mapFocus === item.id; return <div key={item.id} onClick={() => setMapFocus(item.id)} title="Показать на карте" style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", border: `1px solid ${active ? "var(--ac,#b95c3f)" : "var(--line,#e7dcc7)"}`, borderRadius: "var(--r-2)", background: active ? "var(--card,#fff)" : "var(--soft,#fdfaf3)", transition: "border-color .2s, background .2s", cursor: "pointer" }}><span style={{ width: 21, height: 21, flex: "none", borderRadius: "50%", display: "grid", placeItems: "center", background: "var(--ac,#b95c3f)", color: "#fff", fontSize: 11, fontWeight: 700 }}>{index + 1}</span><span style={{ flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: 13, fontWeight: 600 }}>{item.name}</span></div>; })}<div style={{ fontSize: 12, color: "var(--muted,#8a7d6b)", padding: "3px 2px" }}>{mappedRestaurants.length} мест на карте</div></div><RestaurantCityMap restaurants={mappedRestaurants} focus={mapFocus} googleMapsUrl={mapUrl} /></div> : <div style={{ fontSize: 13, color: "var(--muted,#8a7d6b)", padding: "6px 0" }}>В этой категории нет ресторанов с координатами.</div>)}

...
export function RestaurantCityMap({ restaurants, focus, googleMapsUrl }: { restaurants: Restaurant[]; focus: string | null; googleMapsUrl: string }) {
  ...
  useEffect(() => {
   ...
   }, [mapboxToken, googleMapsUrl, restaurants.map((item) => `${item.id}:${item.lnglat}`).join("|")]);

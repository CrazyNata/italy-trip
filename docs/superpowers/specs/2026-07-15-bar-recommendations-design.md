...
- Do not save a rating or review count until Google Maps data is accessible for verification.
...
- Include the name, city, Google Maps URL, coordinates, and a concise Russian note describing the appropriate drink focus.
...
- Research current business details from public map data and cross-check that each place is located in the intended city. Google Maps URLs are included for in-app navigation.
...
- Verify the saved payload contains five added bars per target city, every added record has `placeType: "бар"`, a `"бар"` category, a Maps link, and coordinates.
